...
4. `kubectl apply -f deployment/zk-configmap.yaml` - Set up environment variables for zookeeper
5. `kubectl apply -f deploy/kafka-configmap.yaml` - Set up environment variables for kafka
6. `kubectl apply -f deploy/zk-deploy.yaml` - Set up the deployment for zookeeper.
7. `kubectl apply -f deploy/kafka-deploy.yaml` - Set up the service and deployment for kafka.
8. `kubectl apply -f deploy/location-ingester.yaml` - Set up the deployment for location ingester.
9. `kubectl apply -f deploy/location-server.yaml` - Set up the deployment for location server.
...
13. `python modules/location_client.py` - Sends a sample location payload. Verify this data is inserted into the location table.
