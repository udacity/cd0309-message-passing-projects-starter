...
installFile() {
  ...
  $SUDO cp "$HELM_TMP_BIN" "$HELM_INSTALL_DIR/$BINARY_NAME"
...
[ $EUID = 0 ] || SUDO="sudo -EH" && HELM_INSTALL_DIR=${HELM_INSTALL_DIR:-"/usr/local/bin"}
[ $USE_SUDO = "false" ] && unset SUDO && HELM_INSTALL_DIR=${HELM_INSTALL_DIR:-"$HOME/.local/bin"} && mkdir -p $HELM_INSTALL_DIR
